...
export class SettingsGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleConnection(client: SettingsSocket) {
    try {
      ...
      if (user.role !== 'admin') {
        throw new UnauthorizedException('Bạn không có quyền theo dõi cấu hình hệ thống')
      }

      client.data.userId = user.sub
      await client.join(this.getAdminRoom())
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Xác thực socket thất bại'
      this.logger.warn(`Settings socket rejected: ${message}`)
      client.disconnect(true)
    }
  }

  handleDisconnect(client: SettingsSocket) {
    if (!client.data.userId) {
      return
    }

    void client.leave(this.getAdminRoom())
  }

  emitCleanupLogCreated(payload: { log: Record<string, unknown> }) {
    this.server.to(this.getAdminRoom()).emit('cleanup-log.created', payload)
  }

  private getAdminRoom() {
    return 'settings:admins'
  }

  private extractBearerToken(client: SettingsSocket) {
  ...
  private getHandshakeValue(client: SettingsSocket, key: string) {
